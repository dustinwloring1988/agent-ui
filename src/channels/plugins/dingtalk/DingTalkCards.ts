/**
 * @license
 * Copyright 2025 Agent UI (aionui.com)
...
export function createMainMenuCard(): DingTalkCard {
  return {
    title: 'Agent UI Assistant',
    text: '### Agent UI Assistant\n\nWelcome! Choose an action below:',
    btnOrientation: '1',
...
export function createPairingCard(pairingCode: string): DingTalkCard {
  return {
    ...
    text: ['### Pairing Required', '', 'Please pair your account with Agent UI:', '', `**Pairing Code:** \`${pairingCode}\``, '', '1. Open Agent UI settings', '2. Go to Channels > DingTalk', '3. Enter this pairing code', '', 'Code expires in 10 minutes.'].join('\n'),
    btnOrientation: '1',
...
export function createPairingStatusCard(pairingCode: string): DingTalkCard {
  return {
    ...
    text: ['### Waiting for Approval', '', 'Your pairing request is pending approval.', '', `**Pairing Code:** \`${pairingCode}\``, '', 'Please approve in Agent UI settings:', '1. Open Agent UI app', '2. Go to WebUI > Channels', '3. Click "Approve" for this code'].join('\n'),
    btnOrientation: '1',
...
export function createPairingHelpCard(): DingTalkCard {
  return {
    title: 'Pairing Help',
    text: ['### Pairing Help', '', '**What is pairing?**', 'Pairing links your DingTalk account with the local Agent UI application.', 'You need to pair before using the AI assistant.', '', '**How to pair:**', '1. Send any message to this bot', '2. You will receive a pairing code', '3. Open Agent UI desktop app', '4. Go to WebUI > Channels > DingTalk', '5. Click "Approve" for your code', '', '**FAQ:**', '- Pairing code valid for 10 minutes', '- Agent UI app must be running', '- One account can only pair once'].join('\n'),
...
export function createHelpCard(): DingTalkCard {
  return {
    title: 'Agent UI Assistant Help',
    text: ['### Agent UI Assistant Help', '', 'A remote assistant to interact with Agent UI via DingTalk.', '', '**Common Actions:**', '- New Chat - Start a new session', '- Agent - Switch AI agent', '- Status - View current session status', '- Help - Show this help message', '', 'Send a message to chat with the AI assistant.'].join('\n'),
    btnOrientation: '0',
...
export function createPairingGuideCard(): DingTalkCard {
  return {
    title: 'Pairing Guide',
    text: ['### Pairing Guide', '', '**First-time Setup:**', '1. Send any message to the bot', '2. Bot displays pairing code', '3. Approve pairing in Agent UI settings', '4. Ready to use after pairing', '', '**Notes:**', '- Pairing code valid for 10 minutes', '- Agent UI app must be running', '- One DingTalk account can only pair once'].join('\n'),
    btns: [btn('Back to Help', 'help.show')],
  };
}
...
export function createSettingsCard(): DingTalkCard {
  return {
    title: 'Settings',
    text: ['### Settings', '', 'Channel settings need to be configured in the Agent UI app.', '', 'Open Agent UI > WebUI > Channels'].join('\n'),
